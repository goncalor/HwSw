...
volatile unsigned int *sharedstate =
		(unsigned int *) XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR;
#endif
...
volatile unsigned int *sharedstate1 =
		(unsigned int *) XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + 1;
#endif
...
volatile unsigned int *sharedstate3 =
		(unsigned int *) XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + 2;
...
int main(int argc, char **argv) {
	...
	while ((((*file_aux & 0xFF000000) >> 24 != FILE_END_CODE) && ((*file_aux
			& 0x00FF0000) >> 16 != FILE_END_CODE) && ((*file_aux & 0x0000FF00)
			>> 8 != FILE_END_CODE) && ((*file_aux & 0x000000FF)
			!= FILE_END_CODE)) || (file_aux != end)) {
		putfsl(*file_aux, 0);
		file_aux++;
	}
